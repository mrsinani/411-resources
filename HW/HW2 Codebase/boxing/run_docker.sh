...
# Variables
IMAGE_NAME=boxing_app
CONTAINER_TAG=latest
HOST_PORT=5001
CONTAINER_PORT=5001
DB_VOLUME_PATH="$(pwd)/sql"  # Adjust this to the desired host path for the database persistence
...
if [ ! -d "${DB_VOLUME_PATH}" ]; then
  ...
  mkdir -p "$DB_VOLUME_PATH" # not sure if a specific build command is needed ...
